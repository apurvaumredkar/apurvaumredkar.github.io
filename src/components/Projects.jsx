...
const ProjectCard = ({ title, tech, onClick, delay, colorIndex }) => {
  ...
  return (
    <motion.div
      ...
      onClick={onClick}
      className="bg-gray-50 rounded-3xl p-8 aspect-square flex flex-col justify-center items-center shadow-lg hover:shadow-xl transition-all duration-300 cursor-pointer hover:scale-105 group relative overflow-hidden"
      style={{
    ...
    >
      <h3 className={`text-2xl md:text-3xl font-semibold mb-4 text-center transition-colors duration-300 ${isHovered ? 'text-gray-900' : 'text-gray-800'}`}>{title}</h3>
      <p className={`text-xs font-mono text-center opacity-0 group-hover:opacity-100 transition-opacity duration-300 ${isHovered ? 'text-gray-700' : 'text-gray-500'}`}>{tech}</p>
    </motion.div>
  )
}
...
const Projects = () => {
  ...
  return (
    <section id="projects" className="min-h-screen py-32 px-6 md:px-16 lg:px-32 bg-gradient-to-b from-white to-gray-50 pt-28">
      <motion.h2
        initial={{ opacity: 0 }}
